...
public:
    ...
    void put(const K& key, V value) {
        int index = hash_(key) % TABLE_SIZE;
        std::cout << "Put: key = " << key << ", hash = " << hash_(key) << ", index = " << index << "\n";
...
int main() {
...

    {
        ...
        printf("2. Test remove some key(int).\n");
        {
            for (int i = 0; i < n; ++i) {
                if (i & 1) {
                    mp.erase(i);
...

    {
        ...
        printf("\n3. Test all key(string) in hashmap.\n");
        {
            for (int i = 0; i < n; ++i) {
                if (mp.get(keys[i], value)) {
                    std::cout << "key = " << keys[i] << ", value = " << value << "\n";
                } else {
                    std::cout << "key " << keys[i] << " not existed.\n";
                }
            }
            printf("\n");
        }
